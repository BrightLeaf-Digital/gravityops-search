...
shortcodes—like `gravitymath`, a nested `gravops_search` or any other
shortcode—within the output for each entry.  
...
- Placeholders like `{13}` will be parsed **before** the nested
  shortcode is run
